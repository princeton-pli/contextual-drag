...
declare -A STEPS_MAP=( ["1"]="strategy" ["2"]="filter1"  ["3"]="solve" )
STEP=${STEPS_MAP[$1]}
NUM_PARTITIONS=1
...
for MODEL in "${MODELS[@]}"; do
    ...
    for TASK in "${TASKS[@]}"; do
        ...
        EVAL_DIR="context_denoising/filter1/${STEP}/${TASK}/${MODEL}"

        case "$STEP" in
            solve)
                DATA_PATH="context_denoising/filter1/filter1/${TASK}/${MODEL}/processed_flattened_init_responses.ds"
                case "$TASK" in
                ...
                ;;
            filter1)
                DATA_PATH="context_denoising/filter1/strategy/${TASK}/${MODEL}/processed_flattened_init_responses.ds"
        ...
        JOB_NAME="${TASK}-context-denoising-filter1-${STEP}-N${N_SAMPLES}-${MODEL}"
