...
for MODEL in "${MODELS[@]}"; do
    ...
    for TASK in "${TASKS[@]}"; do
        ...
        TEMPLATE_KEY="2f"
        if [ "$TASK" == "crux-i" ]; then
            TEMPLATE_KEY="2f_crux_input"
        elif [ "$TASK" == "gpqa" ] || [ "$TASK" == "mmlu" ]; then
            TEMPLATE_KEY="2f_qa_mc"
        fi

        JOB_NAME="${TASK}-2f-N${N_SAMPLES}-${MODEL}"
        ...
        bash $launch_script_path \
            ...
            --n-samples $N_SAMPLES \
            --template-path "prompt_templates/2f_templates.json" \
            --template-key $TEMPLATE_KEY \
            --output-dir outputs/2f/${TASK}/${MODEL} \
            --data-path outputs/initial_sampling/${TASK}/2f.ds \
            --num-gpus $NUM_GPUS
    done
done
