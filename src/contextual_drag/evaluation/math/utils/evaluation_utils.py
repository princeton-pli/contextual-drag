...
def evaluate_single_response(response: Dict[str, Any], ground_truth: str, equivalent_parser: Callable) -> Dict[str, Any]:
    ...
    if extracted_answer is None:
        ...
        correctness = equivalent_parser(extracted_answer, ground_truth)
    
    # Add correctness to response (create a copy to avoid modifying the original)
    response_copy = response.copy()
    response_copy['correctness'] = correctness
    response_copy['extracted_answer'] = extracted_answer
    
    return response_copy


def evaluate_single_entry(entry: Dict[str, Any], answer_column: str, response_column: str, equivalent_parser: Callable) -> Dict[str, Any]:
    """
    ...
    """
    ground_truth = entry.get(answer_column)
    ...
    for response in entry[response_column]:
        evaluated_response = evaluate_single_response(response, ground_truth, equivalent_parser)
        entry_copy[response_column].append(evaluated_response)
    
    return entry_copy


def evaluate_responses(dataset: List[Dict[str, Any]], answer_column: str, response_column: str, equivalent_parser: Callable, n_jobs: int = None) -> List[Dict[str, Any]]:
    """
    ...
    evaluated_entries = Parallel(n_jobs=n_jobs, backend='multiprocessing')(
        delayed(evaluate_single_entry)(entry, answer_column, response_column, equivalent_parser) 
...
def analyze_response_errors(dataset: List[Dict[str, Any]], answer_column: str, response_column: str, equivalent_parser: Callable) -> Dict[str, Any]:
    """
    ...
    pass_at_k_stats = pass_and_maj_at_k_by_source(dataset, answer_column, response_column, equivalent_parser)
...
def get_majority_correctness_and_pass_at_k(answer_ls, correctness_ls, equivalent_parser: Callable):
    ...
    for answer in answer_ls:
        ...
        for answer_key in answer_bins:
            if equivalent_parser(answer, answer_key):
...
def compute_majk_and_passk_sliding(answer_ls, correctness_ls, k, equivalent_parser: Callable):
    ...
    for i in range(len(answer_ls)):
        ...
        maj_correctness, pass_at_k = get_majority_correctness_and_pass_at_k(answer_window, correctness_window, equivalent_parser)
        maj_at_k_ls.append(maj_correctness)
        pass_at_k_ls.append(pass_at_k)
    
    return maj_at_k_ls, pass_at_k_ls

def compute_majk_and_passk_stats(answer_ls, correctness_ls, equivalent_parser: Callable):
    pass_at_k_stats = []
    maj_at_k_stats = []
    for k in range(1, len(answer_ls) + 1):
        maj_at_k_ls, pass_at_k_ls = compute_majk_and_passk_sliding(answer_ls, correctness_ls, k, equivalent_parser)
...
def pass_and_maj_at_k_by_source(dataset: List[Dict[str, Any]], answer_column: str, response_column: str, equivalent_parser: Callable) -> Dict[str, Dict[str, Any]]:
    """
    ...
    stats_ls = Parallel(n_jobs=-1, backend='multiprocessing')(
        delayed(compute_majk_and_passk_stats)(answer_ls, correctness_ls, equivalent_parser)
